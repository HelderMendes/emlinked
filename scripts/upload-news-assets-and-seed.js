...
// Helper to upload an image to Sanity CDN asset pipeline
...
// All 18 news items with complete NL and EN translations
const articlesData = [
    {
...
async function seedAllWithAssets() {
    console.log('🚀 Starting Sanity Asset Upload & News Article Seeding...');
    ...
    for (const item of articlesData) {
        ...
        const nlDoc = {
            ...
            title: item.nl.title,
            language: 'nl',
            slug: { _type: 'slug', current: item.nl.slug },
            ...
            seo: {
                seoTitle: `${item.nl.title} | emlinked Nieuws`,
                seoDescription: item.nl.excerpt,
                canonical: `https://emlinked.com/nieuws/${item.nl.slug}`,
                noIndex: false,
            },
            body: item.nl.body.map((paragraph) => ({
                _type: 'block',
                style: 'normal',
        ...
        console.log(`  ✅ Seeded NL: ${item.nl.title} (Asset: ${assetId || 'Fallback'})`);
        ...
        const enDoc = {
            ...
            title: item.en.title,
            language: 'en',
            slug: { _type: 'slug', current: item.en.slug },
            category: item.category,
            ...
            mainImage: imageField,
            seo: {
                seoTitle: `${item.en.title} | emlinked News`,
                seoDescription: item.en.excerpt,
                canonical: `https://emlinked.com/en/news/${item.en.slug}`,
                noIndex: false,
            },
            body: item.en.body.map((paragraph) => ({
                _type: 'block',
                style: 'normal',
                children: [{ _type: 'span', text: paragraph }],
            })),
        ...
        console.log(`  ✅ Seeded EN: ${item.en.title} (Asset: ${assetId || 'Fallback'})`);
    }

    console.log('🎉 Successfully uploaded all image assets and seeded 36 documents (18 NL & 18 EN) into Sanity CMS!');
}
